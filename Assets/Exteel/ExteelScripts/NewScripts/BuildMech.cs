...
public class BuildMech : NetworkBehaviour {
	...
	private string[] defaultParts = {"CES301","AES104","LTN411","HDS003", "PBS000"};
	...
	void CmdRegister(uint id, Data d) {
		...
		if (registeredPlayers == connectedPlayers){
			foreach (KeyValuePair<GameObject, Data> entry in gm.playerInfo){
				...
				mechBuilder.RpcBuildMech(m.Core, m.Arms, m.Legs, m.Head, m.Booster);
				mechBuilder.buildMech(m.Core, m.Arms, m.Legs, m.Head, m.Booster);
			}
	...
	[ClientRpc]
	public void RpcBuildMech(string c, string a, string l, string h, string b){
		buildMech(c,a,l,h,b);
	}

	public void buildMech(string c, string a, string l, string h, string b) {

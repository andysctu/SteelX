...
public class ChatNewGui : MonoBehaviour, IChatClientListener
{
    ...
    //public Toggle ChannelToggleToInstantiate; // set in inspector //TODO : implement this
	//public GameObject FriendListUiItemtoInstantiate;
	
    //private float registerPlayerTimer, RegisterPlayerInterval = 3, updatePlayerListTimeer , updatePlayerListInterval = 5;
	...
	public void Init()
	{
		...
		if (string.IsNullOrEmpty(UserName))UserName = PhotonNetwork.player.NickName; //made-up username
	...
	public void OnSubscribed(string[] channels, bool[] results)
	{
		...
		{
			this.chatClient.PublishMessage(channel, "entered the lobby"); // you don't HAVE to send a msg on join but you could.
    ...
    List<string> RegisterCommands = new List<string>();
    ...
    public void ShowChannel(string channelName)
	{
        ...
        //    string[] tempLines = channel.ToStringMessages().Split(Environment.NewLine.ToCharArray());
        //    for(int i = 0; i < tempLines.Length; i++) {
        //        if (tempLines[i].Contains("@!") || tempLines[i] == "") {
        //            if(tempLines[i] != "") {
        //                RegisterCommands.Add(tempLines[i]);
        //            }
        //            tempLines[i] = "";
        //        } else {
        //            tempLines[i] += '\n';
        //        }
        //    }
        //    CurrentChannelText.text = string.Concat(tempLines);
        //}
    ...
    //private void RegisterCommandsToPlayerList() {
    //    for(int i = 0; i < RegisterCommands.Count; i++) {
    //        //Trim name from string
    //        int j;
    //        for (j=0;j< RegisterCommands[i].Length; j++) {
    //            if(RegisterCommands[i][j] == ':') {
    //                break;
    //            }
    //        }
    //        string name = RegisterCommands[i].Substring(0, j);
    //        Debug.Log("name : "+name);
    //    }
    ...
    //private void FixedUpdate() {
        //if (Mathf.Abs(registerPlayerTimer - Time.time) > RegisterPlayerInterval) {
        //    registerPlayerTimer = Time.time;
        //    SendChatMessage("@!");
        //}

        //if(Mathf.Abs(updatePlayerListTimeer - Time.time) > updatePlayerListInterval){
        //    updatePlayerListTimeer = Time.time;
        //    RegisterCommandsToPlayerList();
        //}
    //}
